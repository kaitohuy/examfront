...
export class NotificationService {
  ...
  list(page = 0, size = 10): Observable<PageResult<Notification>> {
    const params = new HttpParams().set('page', page).set('size', size);
    return this.http.get<PageResult<Notification>>(this.base, { params });
  }
  ...
  markAllRead(): Observable<number> { // trả về count đã đọc
